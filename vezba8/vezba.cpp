...
char tocniPrvi[4]={'b', 'a', 'b', 'a'};
char tocniVtori[4]={'a', 'b', 'a', 'a'};
char tocniTreti[4][2]={{'a', 'c'}, {'a', 'b'}, {'a', 'c'}, {'b', 'c'}};
    ...
    if (brojprasanje <= 4){
      ...
      writeOdgovor(file, odgovor, ' ', brojprasanje);
        ...
        do{
          cout<<"Vashiot odgovor: ";
          cin>>odgovor2;
        } while(( odgovor2 == odgovor )  || ( odgovor2 != 'a' && odgovor2 != 'b' && odgovor2 != 'c'));
...
int main(){
	...
	// Proverva dali postoi veke takov fajl
    string nameOfFile = lice.ime + lice.prezime + "INKI" + lice.indeks;
	file.open(nameOfFile.c_str());
    if (file)
    {
        cout << "Nemate pravo povtorno da go polagate testot." << endl;
        abort();
    }
    ...
    time_t now = time(0);
    char* dt = ctime(&now);
    ofstream students("ucenik.txt", ios_base::app | ios_base::out);
    students << lice.ime + " " + lice.prezime + " INKI" + lice.indeks + " - " + dt;
    students << endl;
